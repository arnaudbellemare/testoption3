...
import numpy as np
import ccxt
from toolz.curried import *  # Provides functional tools like get_in
...
# Default expiry and time to expiry calculation
...
###########################################
# Risk Adjustment: Cornish-Fisher Expansion
###########################################
def compute_risk_adjustment_factor_cf(df, alpha=0.05):
    """
    Computes a risk adjustment factor using the Cornish-Fisher expansion.
    This adjustment accounts for skewness and kurtosis in the return distribution,
    ensuring that fat tails and asymmetry (non-normal behavior) are reflected in risk measures.
    ...
    Returns:
      risk_factor: A multiplier (>1 implies volatility scaling up).
    """
    df = df.copy()
    ...
    K = returns.kurtosis() + 3  # Convert excess kurtosis to full kurtosis
...
def load_credentials():
    try:
        ...
        if len(usernames) != len(passwords):
            st.error("Mismatch in number of usernames and passwords.")
            return {}
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Login"):
            ...
            if username in creds and creds[username] == password:
                ...
                st.success("Logged in successfully! Click the login button again to open the app!")
            else:
...
def get_actual_iv(instrument_name):
    ...
    if response.status_code != 200:
        return None
    data = response.json()
    # Using toolz.get_in to extract nested data safely
...
###########################################
# Kraken Data Fetching (Dual Timeframe)
###########################################
...
###########################################
# Volatility Calculation: EWMA-Roger Satchell
###########################################
def calculate_ewma_roger_satchell_volatility(price_data, span=30):
    ...
    This estimator captures both the upward and downward price movements, making it
    effective even when there is a volatility skew.
    """
    df = price_data.copy()
...
def compute_daily_realized_volatility(df, span=30, annualize_days=365):
    """
    Resample the data daily using OHLC, compute the EWMA-Roger Satchell volatility,
    and annualize it.
    """
...
###########################################
# Daily Average Implied Volatility and VRP
###########################################
...
###########################################
# Option Delta, Gamma, and Gamma Exposure (GEX)
###########################################
def compute_delta(row, S):
    ...
    if sigma <= 0:
        return np.nan
    sigma_eff = sigma * (globals().get("risk_factor", 1))
    try:
...
###########################################
# EV Calculation Functions for Various Strategies
###########################################
def adjust_ev(ev_value, position_side):
    # Adjust the EV based on whether the position is long or short
...
def calculate_atm_straddle_ev(ticker_list, spot_price, T, rv, position_side="short"):
    tolerance = spot_price * 0.02  # Narrow range around the spot for ATM
    ...
    for item in atm_candidates:
        strike = item["strike"]
        atm_strikes.setdefault(strike, {"iv_sum": 0, "count": 0})
    ...
    for strike, data in atm_strikes.items():
        ...
        ev_value = ev_value / globals().get("risk_factor", 1)
...
def calculate_limited_otm_put_ev(ticker_list, spot_price, T, rv, position_side="long"):
    tolerance = spot_price * 0.10  # Wider tolerance for puts
    ...
    for item in candidates:
        strike = item["strike"]
        group.setdefault(strike, {"iv_sum": 0, "count": 0})
...
def calculate_small_atm_straddle_ev(ticker_list, spot_price, T, rv, position_side="short"):
    # Reuse the ATM straddle calculation
    return calculate_atm_straddle_ev(ticker_list, spot_price, T, rv, position_side)

###########################################
# Dynamic Volatility Adjustment Using Smile
###########################################
...
###########################################
# Building the Smile DataFrame and Ticker List
###########################################
...
def evaluate_trade_strategy(df, spot_price, risk_tolerance="Moderate", df_iv_agg_reset=None,
    ...
    # Compute realized volatility using EWMA-Roger Satchell
...
def main():
    ...
    # Compute risk adjustment factor using Cornish-Fisher expansion
    global risk_factor
